...
$(document).ready(function() {
	...
	$('select#page').change(function () {
		...
		var patternGroupId = $(this).data('pattern-group-id');
		...
		if (searchText && searchText.length > 0 && searchText !== 'undefined') {
			url = Routing.generate('mobile_front_search', {search: searchText, page: page});
		} else if (promotionId) {
			url = Routing.generate('mobile_front_promotion', {id: promotionId, page: page});
		} else {
			url = Routing.generate('mobile_front_portfolio', {brand_id:brandId, pattern_group_id: patternGroupId, page: page});
		}
	...
	$('input[name="search"]').focusin(function () {
		$('button#search').html('<i class="fa fa-hand-o-right font-16"></i>');
	}).focusout(function () {
		$('button#search').html('<i class="fa fa-search font-16"></i>');
	});
	...
	var mobileAvenueCart = new MobileAvenueCart();
    ...
    //Initiate Image Sliders
	var owlStaffControls = $(".product-slider");
	owlStaffControls.owlCarousel({
		//Basic Stuff
		items : 1,
		itemsDesktop : [1199,1],
