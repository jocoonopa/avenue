...
auctionCtrl.controller('PunchOutCtrl', ['$scope', '$routeParams', '$http',
  ...
  $scope.addPunchOutList = function () {
    if (0 < $scope.punchOutBarcode.length) {
  ...
  $scope.savePunchOutList = function () {
    $scope.successPunchOutBarcodes  = $scope.punchOutBarcodes;
    $scope.failPunchOutBarcodes     = ['ABC','CD','EFG'];
    $scope.punchOutBarcodes         = [];
  };
